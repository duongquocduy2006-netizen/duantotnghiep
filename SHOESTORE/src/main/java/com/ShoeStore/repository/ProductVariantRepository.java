...
public interface ProductVariantRepository extends JpaRepository<ProductVariant, Integer> {
    ...
    // Đếm số order_items thuộc đơn hàng đang xử lý (status NOT IN (3, 4) — chỉ cho xóa biến thể khi đơn đã hoàn tất hoặc đã hủy)
    @org.springframework.data.jpa.repository.Query(value = "SELECT COUNT(*) FROM order_items oi " +
            "JOIN orders o ON oi.order_id = o.id " +
            "WHERE oi.product_variant_id = ?1 AND o.status NOT IN (3, 4)", nativeQuery = true)
